module.exports = function(context) {
    ...
    var wxpayKeysPath = wxpayPath + '/Keyswx.java';

    fs.readFile(context.opts.projectRoot + '/plugins/nova.cordova.plugins.wxpay/src/android/com/nova/cordova/wxpay/Keyswx.java', 'utf8', function(err, data) {
        if (err) throw err;
        var result = data.replace(/\$APPID/g, androidJSON.installed_plugins["nova.cordova.plugins.wxpay"].APPID);
        result = result.replace(/\$MCHID/g, androidJSON.installed_plugins["nova.cordova.plugins.wxpay"].MCHID);
        result = result.replace(/\$APIKEY/g, androidJSON.installed_plugins["nova.cordova.plugins.wxpay"].APIKEY);
    ...
    fs.readFile(context.opts.projectRoot + '/plugins/nova.cordova.plugins.wxpay/src/android/WXPayEntryActivity.java', 'utf8', function(err, data) {
        if (err) throw err;
